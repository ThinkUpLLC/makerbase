        ...
        <div id="maker-info-profile">
          ...
          {if isset($logged_in_user)}
            {if !$maker->is_archived && !$is_maker_user eq true}
             <div id="unfollow{$maker->uid}" class="follow-button {if $logged_in_user->is_following_maker eq false}hidden{/if}" ><a class="btn btn-xs btn-default btn-unfollow" uid="{$maker->uid}" type="maker" data-loading-text="<i class='fa fa-spinner fa-spin'></i> Unfollowing">Following</a></div>
             <div id="follow{$maker->uid}" class="follow-button {if $logged_in_user->is_following_maker eq true}hidden{/if}" ><a class="btn btn-xs btn-info btn-follow" uid="{$maker->uid}" type="maker" data-loading-text="<i class='fa fa-spinner fa-spin'></i> Following">Follow</a></div>
            {/if}
          {else}
