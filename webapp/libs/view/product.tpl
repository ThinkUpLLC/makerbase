...
<div class="row">
  ...
  <nav>
    <ul class="pager">
      {if isset($next_page)}
        <li class="previous"><a href="/p/{$product->uid}/{$product->slug}/{$next_page}"><span aria-hidden="true">&larr;</span> Older</a></li>
      {/if}
      {if isset($prev_page)}
        <li class="next"><a href="/p/{$product->uid}/{$product->slug}{if $prev_page neq 1}/{$prev_page}{/if}">Newer <span aria-hidden="true">&rarr;</a></li>
      {/if}
    </ul>
  </nav>
  ...
  {if isset($logged_in_user)}
      ...
      <form method="post" action="/add/role/" class="form-horizontal">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9 col-xs-8">
              <input type="text" class="form-control" id="role" autocomplete="off" name="role" placeholder="{$placeholder}">
            </div>
          </div>
