...
--
ALTER TABLE users
ADD has_added_maker INT(1) NOT NULL DEFAULT NULL COMMENT 'Whether user has added a maker.' ,
ADD has_added_product INT(1) NOT NULL DEFAULT NULL COMMENT 'Whether user has added a product.' ,
ADD has_added_role INT(1) NOT NULL DEFAULT NULL COMMENT 'Whether user has added a role.' ;
...
-- Fun usage stats!
--
select count(*) as added_product_but_no_maker from users where has_added_product = 1 and has_added_maker = 0;

select count(*) as added_maker_but_no_product from users where has_added_product = 0 and has_added_maker = 1;

select count(*) as added_maker_product_but_no_role from users where has_added_product = 1 and has_added_maker = 1 and has_added_role = 0;